...
class _ForgotCodeScreenState extends State<ForgotCodeScreen>
  ...
  Future<bool> _validateCIFAndPhone() async {
    try {
      final prefs = await SharedPreferences.getInstance();
      final cif = _extractCIF(_accountController.text);
      final savedPhone = prefs.getString('phone_$cif');

      final registeredCIFs = prefs.getStringList('registered_cifs') ?? [];
      final isRegistered = registeredCIFs.contains(cif);

      return isRegistered && savedPhone == _phoneController.text;
    } catch (e) {
      debugPrint('Error validating CIF and phone: $e');
      return false;
    }
  }
  ...
  void _proceedToOTP() async {
    ...
    try {
      ...
      final isValid = await _validateCIFAndPhone();

      if (!isValid) {
        setState(() => _isLoading = false);
        _showErrorSnackBar(
            'Account number and phone number do not match or account not registered');
        return;
      }
  ...
  Widget _buildHeader() {
    return Column(
      children: [
        ...
        Text(
          'Enter your account details to reset your access code',
          textAlign: TextAlign.center,
          style: TextStyle(
